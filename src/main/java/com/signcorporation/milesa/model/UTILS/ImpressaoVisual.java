 ...
 */
public class ImpressaoVisual extends Stage {
    ...
    protected double resolucaoECRAN = REPORT_DPI_RESOLUTION;
    ...
    public ImpressaoVisual(JasperPrint jasperPrint) {
        this(jasperPrint, null);
    }

    public ImpressaoVisual(JasperPrint jasperPrint, ResourceBundle resBundle) {
        this(DefaultJasperReportsContext.getInstance(), jasperPrint, resBundle);
    }

    public ImpressaoVisual(JasperReportsContext jasperReportsContext, JasperPrint jasperPrint, ResourceBundle resBundle) {
        this.jasperReportsContext = jasperReportsContext;
        this.jasperPrint = jasperPrint;

        initResources(resBundle);
        resolucaoECRAN = Screen.getPrimary().getDpi();
        initComponents();
        setZoomRatio(1);
    ...
    private void initComponents() {
        ...
        btnActualSize.setOnAction(e -> {
            if (((Toggle) e.getSource()).isSelected()) {
                onActualSizePressed();
            }
        });
    ...
    protected void atualizar() {
        ...
        for (int pageIndex = 0; pageIndex < jasperPrint.getPages().size(); pageIndex++) {
            try {
                BufferedImage awtImage = (BufferedImage) JasperPrintManager.getInstance(jasperReportsContext).printToImage(jasperPrint, pageIndex, realZoom);
    ...
    protected void setZoomRatio(float newZoom) {
        if (newZoom > 0) {
            ...
            if (zoom != newZoom) {
                zoom = newZoom;
                realZoom = (float) (zoom * resolucaoECRAN / REPORT_DPI_RESOLUTION);
                atualizar();
            }
        }
    ...
    private void setRealZoomRatio(float newZoom) {
        if (newZoom > 0 && realZoom != newZoom) {
            zoom = (float) (newZoom * REPORT_DPI_RESOLUTION / resolucaoECRAN);
            realZoom = newZoom;
