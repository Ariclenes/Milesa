...
public class ProdutoDAO implements CRUD<Produto> {
    ...
    public  void inserir(Produto p) throws ExceptionValidacao, SQLException {
       String SQL  = "INSERT INTO PRODUTO (CODBARRA,NOME_PRODUTO,PCM,PVP,ESTOQUE_MINIMO,ESTOQUE_MAXIMO,FOTO_PRODUTO, TIPO_PRODUTO, CODSUBGRUPO, CODCATEGORIA,  CODUN_MEDIDA,   "
                     + "CONTROLA_ESTOQUE,MODIFICADO )values (?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP())";
        try {
      PreparedStatement  ps = conexao.prepareStatement(SQL);
        ps.setString(1, p.getCODBARRA ());
        ps.setString(2, p.getNOME_ARTIGO());
        ...
        ps.setInt(11, p.getUNIDADE_MEDIDA().getCodUnidade ());
//        ps.setString (12, p.getSTATUS ().getSIGLA ());
        ps.setBoolean (12, p.isCONTROLA_ESTOQUE ());
//        con.confirmar ();
...
//          connetion.close();
    ...
    public  void alterar(Produto p) throws ExceptionValidacao, SQLException {
         String sql = "UPDATE PRODUTO SET NOME=?,  DESCRICAO=? CA WHERE CODPRODUTO=?";
...
//                  + "ORDER BY NOME_PRODUTO LIMIT 100";
		...
		while (rs.next()) {
                ...
                p.setPRODUTO_CATEGORIA (ProdutoCategoriaDAO.get(rs.getInt("CODCATEGORIA")));
...
//                p.setPRECO_COMPRA (rs.getDouble("PCM"));
//                p.setPRECO_VENDA (rs.getDouble("PVP"));
//                p.setPRODUTO_CATEGORIA (ProdutoCategoriaDAO.get(rs.getInt("CATEGORIA")));
//                p.setPRODUTO_TIPO (ProdutoTipoDAO.get(rs.getInt("TIPO_PRODUTO")));
